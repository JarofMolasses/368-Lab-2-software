...
namespace Lab2
{
    public partial class Form2 : Form
    {
        ...
        ConcurrentQueue<float> tempQueue = new ConcurrentQueue<float>();
        ConcurrentQueue<float> tempAvgQueue = new ConcurrentQueue<float>();
        ...
        float[] tempDisplayCopyArray = new float[tempWindowSize];
        ...
        {
            serialPort1.PortName = "COM3";          // lazy, this is the default MSP430 port 
            ...
            if(serialPort1.IsOpen)
            {
                foreach(Int32 bufferItem in bufferQueue)
                {
                    ...
                    switch (byteState)
                    {
                        ...
                        case 2:
                            ls5b = bufferItem;                   // LSB: add to sum
                            Console.Write($"LS5B={Convert.ToString(bufferItem, 10)}, ");

                            int adcReading = (ms5b << 5) | ls5b;          // add MSB << 5 and LSB 
                            float voltage = Vcc * (adcReading / (float)analogRes);
                            voltsQueue.Enqueue(voltage);
                            thermistorVDisplay.Text = voltage.ToString();

                            float temp = convertVoltsToTemperature(voltage);
                            temperatureKDisplay.Text = temp.ToString();
                            temperatureCDisplay.Text = (temp - 273.15).ToString();
                            tempQueue.Enqueue(temp);

                            float tempAvg = tempQueue.Average();
                            tempKRollingAvgDisplay.Text = tempAvg.ToString();
                            tempAvgQueue.Enqueue(tempAvg);
                            ...
                            {
                                tempAvgQueue.TryDequeue(out trash);
                            }
        ...
        private void updateRollingAvgSize()
        {
